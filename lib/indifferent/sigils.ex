defmodule Indifferent.Sigils do
  ...
  defmacro sigil_I(sigil, modifiers) do
    ...
    quote do
      [fn
        :get, data, next ->
          unquote(__MODULE__).get_in(data, next, unquote(path), unquote(modifiers))
        :get_and_update, data, next ->
          unquote(__MODULE__).get_and_update_in(data, next, unquote(path), unquote(modifiers))
      end]
    end
  end
  ...
  defmacro sigil_i(data, sigil, modifiers) do
    path = sigil_to_quoted!(sigil, __CALLER__) |> Macro.escape
    quote do
      unquote(__MODULE__).get_in(unquote(data), fn x -> x end, unquote(path), unquote(modifiers))
    end
  end
